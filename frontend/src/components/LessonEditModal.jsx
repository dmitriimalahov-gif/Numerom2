...
import { Edit, Save, X, Plus, Trash2, BookOpen, Brain, Users, FileText, BarChart3, Upload, Calendar, Eye, Download, ExternalLink, Clock } from 'lucide-react';
...
const LessonEditModal = ({ 
  ...
  const [exercisesMediaFiles, setExercisesMediaFiles] = useState([]);
  const [exercisesDocuments, setExercisesDocuments] = useState([]);
  const [challengeMediaFiles, setChallengeMediaFiles] = useState([]);
  const [challengeDocuments, setChallengeDocuments] = useState([]);
  const [quizMediaFiles, setQuizMediaFiles] = useState([]);
  ...
  // Текущий раздел на основе активной вкладки
  const currentSection = activeTab === 'theory' ? 'theory' :
                        activeTab === 'exercises' ? 'exercises' :
                        activeTab === 'challenge' ? 'challenge' :
                        activeTab === 'quiz' ? 'quiz' : 'theory';

  // Функции для получения файлов текущего раздела
  const getCurrentMediaFiles = () => {
    switch (currentSection) {
      case 'theory': return theoryMediaFiles;
      case 'exercises': return exercisesMediaFiles;
      case 'challenge': return challengeMediaFiles;
      case 'quiz': return quizMediaFiles;
      default: return theoryMediaFiles;
    }
  };

  const getCurrentDocumentFiles = () => {
    switch (currentSection) {
      case 'theory': return theoryDocuments;
      case 'exercises': return exercisesDocuments;
      case 'challenge': return challengeDocuments;
      case 'quiz': return quizDocuments;
      default: return theoryDocuments;
    }
  };
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const analyticsResponse = await fetch(
        `${backendUrl}/api/admin/analytics/lesson/${lesson.id}`,
        {
          headers: {
      ...
      const responsesResponse = await fetch(
        `${backendUrl}/api/admin/analytics/student-responses/${lesson.id}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
      ...
      const notesResponse = await fetch(
        `${backendUrl}/api/admin/analytics/challenge-notes/${lesson.id}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
      ...
      const filesResponse = await fetch(
        `${backendUrl}/api/admin/lesson-files-analytics/${lesson.id}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
  ...
  const handleReviewResponse = async (responseId) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/admin/review-response/${responseId}`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          ...
          body: JSON.stringify({ 
            ...
            points_earned: exercisePoints || 0,
            review_start_time: reviewStartTime ? reviewStartTime.toISOString() : null
          })
        }
      );
      ...
      if (response.ok) {
        ...
        setStudentResponses(prev => prev.map(r => 
          r.id === responseId 
            ? { 
                ...
                reviewed: true, 
                admin_comment: adminComment,
                points_earned: data.points_earned || exercisePoints || 0,
                review_time_minutes: data.review_time_minutes || 0
              }
            : r
        ));
        ...
        alert(`Комментарий добавлен${data.points_earned > 0 ? `, начислено ${data.points_earned} баллов` : ''}`);
      } else {
  ...
  const loadFiles = async (section) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/admin/files?lesson_id=${lesson.id}&section=${section}`,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        if (section === 'theory') {
          ...
          setExercisesMediaFiles(mediaFiles);
          setExercisesDocuments(documents);
        } else if (section === 'challenge') {
          setChallengeMediaFiles(mediaFiles);
          setChallengeDocuments(documents);
        } else if (section === 'quiz') {
          setQuizMediaFiles(mediaFiles);
          setQuizDocuments(documents);
        }
      }
    } catch (error) {
  ...
  const handleFileUpload = async (event, section, fileType) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const response = await fetch(
          `${backendUrl}/api/admin/lessons-v2/${lesson.id}/upload-file`,
          {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleFileDelete = async (fileId, section) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/api/admin/files/${fileId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  // Загрузка файлов при открытии вкладки
  useEffect(() => {
    if (lesson?.id) {
      if (activeTab === 'theory') {
        loadFiles('theory');
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col px-8">
          ...
          <div className="flex-1 overflow-y-auto py-6 pr-2" style={{ scrollbarWidth: 'thin' }}>
            ...
            <TabsContent value="theory" className="space-y-4 mt-0">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    accept="image/*,video/*"
                    multiple
                    onChange={(e) => handleFileUpload(e, currentSection, 'media')}
                    className="hidden"
                  />
                  <Button 
                  ...
                  <input
                    ...
                    multiple
                    onChange={(e) => handleFileUpload(e, currentSection, 'document')}
                    className="hidden"
                  />
                  <Button 
              ...
              {/* ЗАГРУЖЕННЫЕ ФАЙЛЫ */}
              {(getCurrentMediaFiles().length > 0 || getCurrentDocumentFiles().length > 0) && (
                <div className="mt-6 space-y-4">
                  {/* Медиафайлы */}
                  {getCurrentMediaFiles().length > 0 && (
                    <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                      <h4 className="text-sm font-semibold text-purple-900 mb-3 flex items-center gap-2">
                        <Upload className="w-4 h-4" />
                        Медиафайлы ({getCurrentMediaFiles().length})
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {getCurrentMediaFiles().map((file) => (
                          <div key={file.id} className="bg-white p-3 rounded border border-purple-200 flex justify-between items-center">
                            ...
                            <div className="flex gap-1 ml-2">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => handleFileDelete(file.id, currentSection)}
                                className="text-red-600 hover:text-red-700 hover:bg-red-50"
                                title="Удалить"
                              >
                                <Trash2 className="w-4 h-4" />
                  ...
                  {/* Документы */}
                  {getCurrentDocumentFiles().length > 0 && (
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                      <h4 className="text-sm font-semibold text-green-900 mb-3 flex items-center gap-2">
                        <FileText className="w-4 h-4" />
                        Документы ({getCurrentDocumentFiles().length})
                      </h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {getCurrentDocumentFiles().map((file) => (
            ...
            <TabsContent value="analytics" className="space-y-4 mt-0">
              {loadingAnalytics ? (
                ...
                <>
                  ...
                  {analyticsData.statistics.time_analytics && (
                    <div className="bg-white p-5 rounded-lg border border-gray-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                          <p className="text-xs text-gray-600 mb-1">Время на чтение теории</p>
                          <p className="text-xl font-bold text-blue-600">
                            {analyticsData.statistics.time_analytics.theory_time_hours}ч
                          </p>
                          <p className="text-xs text-gray-500 mt-1">
                            {analyticsData.statistics.time_analytics.theory_time_minutes} минут
                          </p>
                        </div>
                        <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                          <p className="text-xs text-gray-600 mb-1">Время на выполнение заданий</p>
                          <p className="text-xl font-bold text-green-600">
                            {analyticsData.statistics.time_analytics.exercise_time_hours}ч
                          </p>
                          <p className="text-xs text-gray-500 mt-1">
                            {analyticsData.statistics.time_analytics.exercise_time_minutes} минут
                          </p>
                        </div>
                        <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                          <p className="text-xs text-gray-600 mb-1">Время проверки администратором</p>
                          <p className="text-xl font-bold text-purple-600">
                            {analyticsData.statistics.time_analytics.review_time_hours}ч
                          </p>
                          <p className="text-xs text-gray-500 mt-1">
                            {analyticsData.statistics.time_analytics.review_time_minutes} минут
                          </p>
                        </div>
                        <div className="bg-orange-50 p-3 rounded-lg border border-orange-200">
                          <p className="text-xs text-gray-600 mb-1">Среднее время на урок</p>
                          <p className="text-xl font-bold text-orange-600">
                            {analyticsData.statistics.time_analytics.avg_lesson_time_hours}ч
                          </p>
                          <p className="text-xs text-gray-500 mt-1">
                            {analyticsData.statistics.time_analytics.avg_lesson_time_minutes} минут
                          </p>
                        </div>
                      </div>
                    </div>
                        ...
                        {analyticsData.challenge_leaderboard.map((student, index) => (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-sm">{student.user_name || `Студент ${student.user_id.substring(0, 8)}`}</p>
                                <p className="text-xs text-gray-500">
                          ...
                          >
                            ...
                            {response.reviewed && (
                              <div className="space-y-2">
                                ...
                                {(response.points_earned > 0 || response.review_time_minutes > 0) && (
                                  <div className="flex items-center gap-4 p-2 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded">
                                    {response.points_earned > 0 && (
                                      <div className="flex items-center gap-1">
                                        <span className="text-xs font-semibold text-yellow-800">Баллы:</span>
                                        <span className="text-sm font-bold text-yellow-600">{response.points_earned}</span>
                                      </div>
                                    )}
                                    {response.review_time_minutes > 0 && (
                                      <div className="flex items-center gap-1">
                                        <Clock className="w-3 h-3 text-gray-600" />
                                        <span className="text-xs text-gray-600">
                                          Время проверки: {Math.round(response.review_time_minutes)} мин
                                        </span>
                                      </div>
                                    )}
                                  </div>
                              ...
                              <div className="mt-3">
                                {reviewingResponse === response.id ? (
                                  <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => setExercisePoints(parseInt(e.target.value) || 0)}
                                    ...
                                    <div className="flex gap-2">
                                      ...
                                      >
                                        Сохранить комментарий{exercisePoints > 0 ? ` и ${exercisePoints} баллов` : ''}
                                      </Button>
                                      <Button 
                                        size="sm" 
                                        variant="outline"
                                  ...
                                  <Button 
                                    ...
                                    variant="outline"
                                    onClick={() => {
                                      setReviewingResponse(response.id);
                  ...
                  {challengeNotes.length > 0 ? (
                    ...
                    <div className="bg-white p-5 rounded-lg border border-gray-200">
                      ...
                      <p className="text-sm text-gray-500">Заметок по челленджу пока нет.</p>
                    </div>
                  )}
                  ...
                  {filesAnalytics && filesAnalytics.files && filesAnalytics.files.length > 0 ? (
                    ...
                    <div className="bg-white p-5 rounded-lg border border-gray-200">
                      ...
                      <p className="text-sm text-gray-500">Статистика по файлам пока отсутствует.</p>
                    </div>
                  )}
                </>
              ) : (
            ...
            <div className="flex-1 overflow-auto p-6 bg-gray-50">
              ...
              {viewingFile.mime_type?.startsWith('image/') && (
                <div className="flex items-center justify-center h-full">
                  <img
                    src={`${backendUrl}/api/download-file/${viewingFile.id}`}
